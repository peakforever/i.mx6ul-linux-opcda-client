...
远程 DA 访问的三要素,配置时按此核对:

1. **配置用户**:创建有非空密码的测试账号,本地安全策略设
   "经典 - 对本地用户进行身份验证";DCOM 组件身份选"指定用户"。
   空密码账号远程访问会被拒绝。
2. **配置防火墙放行**:入站放行 TCP 135(DCOM 激活/OPCEnum)+
   RPC 动态端口 49152-65535(业务通道,或注册表固定端口只放行该段)。
   能 ping 通但连不上,~90% 是这一条漏了。
3. **配置组件允许远程激活与运行**:必须用 32 位控制台
   `mmc comexp.msc /32`;对 OPCEnum 和具体 Server 两个组件的
   "启动和激活权限"与"访问权限"添加账号并勾选**远程**启动/激活/访问。

配置顺序建议:先 1、3(账号+权限),再 2(防火墙)——防火墙是最后一道闸,
前面配好放行即通;反过来排查时难以区分是哪一层的问题。
